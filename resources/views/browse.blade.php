                    ...
                    @foreach($recipes as $recipe)
                        ...
                        <div class="bg-white overflow-hidden shadow-md rounded-lg hover:shadow-lg transition-shadow duration-base border border-gray-200 h-full flex flex-col cursor-pointer" onclick="window.location.href='{{ route('recipes.show', $recipe) }}'">
                            ...
                            <div class="p-5 flex flex-col flex-grow">
                                ...
                                {{-- Like Percentage - Fixed position with min-height --}}
                                <div class="flex items-center gap-1 text-secondary-500 mb-3 h-6">
                                ...
                                @auth
                                    @php
                                        $isSaved = auth()->user()->savedRecipes()->where('recipe_id', $recipe->id)->exists();
                                    @endphp
                                    <div class="flex gap-2 mt-auto">
                                        <span class="flex-grow items-center justify-center px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white text-sm font-medium rounded-md transition-colors duration-base text-center">
                                            View Recipe
                                        </span>
                                        @if($isSaved)
                                            {{-- Unsave button --}}
                                        ...
                                        @else
                                            {{-- Save button --}}
                                            <form action="{{ route('recipes.save', $recipe) }}" method="POST" onclick="event.stopPropagation()">
                                                @csrf
                                ...
                                @else
                                    {{-- Guest users see only View Recipe --}}
                                    <div class="mt-auto">
                                        <span class="block w-full text-center px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white text-sm font-medium rounded-md transition-colors duration-base">
                                            View Recipe
                                        </span>
                                    </div>
                                @endauth
